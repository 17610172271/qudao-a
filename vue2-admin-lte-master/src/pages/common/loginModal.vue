<template>
	<div class="login-modal-container">
		...
		<div><button class="login-btn" @click="submit">登陆</button></div>
		<div class="login-other text-lg clear m-t-lg">
			<label class="pull-left">
				<input type="checkbox">
				<span>记住密码</span>
			</label>
			<router-link :to="{name: 'login', query: {action: 'password'}}" class="pull-right">忘记密码</router-link>
			<router-link :to="{name: 'login', query: {action: 'register'}}" class="pull-right m-r-sm">注册</router-link>
		</div>
	</div>
</template>
<script>
...
export default {
  ...
  computed: {
  	codeImg () {
  		return 'http://www.agent_api.com/v1/captcha/get?id=' + this.id
  	}
  },
  methods: {
  	updateId () {
